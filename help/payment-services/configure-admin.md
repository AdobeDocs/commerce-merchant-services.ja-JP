...
1. 表示する [!DNL Apple Pay] 「チェックアウト」ページで、「 」を選択します。 `Yes` （の） **[!UICONTROL Show buttons on checkout page]**.
1. 表示する [!DNL Apple Pay] 製品の詳細ページで、「 」を選択します。 `Yes` （の） **[!UICONTROL Show buttons on product detail page]**.
1. 表示する [!DNL Apple Pay] ミニカートのプレビューで、「 」を選択します。 `Yes` 対象： **[!UICONTROL Show buttons in mini cart preview]**.
1. 表示する [!DNL Apple Pay] 買い物かごページで、「 」を選択します。 `Yes` （の） **[!UICONTROL Show buttons on cart page]**.
