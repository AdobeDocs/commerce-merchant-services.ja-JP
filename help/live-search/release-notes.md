...
* 新しいカテゴリマーチャンダイジングタブが追加されました。 ユーザーは、カテゴリごとにインテリジェントランキングと手動ランキング（ピン、ブースト、ベリー、非表示）を追加できるようになりました。
* ユーザーは、インテリジェントランキングまたは手動ランキングを持つ単一のカテゴリルールを追加できます
* ユーザーは、インテリジェントランキングルールをサブカテゴリに追加できるようになりました
* インテリジェントランキングを持つサブカテゴリを削除する際に、詳細情報が表示されます
* 継承されたランキング戦略のルールを削除する機能を追加しました。
* 単一のカテゴリのルールを削除する機能を追加しました。
* ルールを追加する際に、カテゴリ名で検索できるようになりました
* カテゴリツリー表示を使用すると、ルールが適用されたカテゴリを表示できるようになりました。
* カテゴリプレビューには、選択したカテゴリのみが表示されます。

### 更新

* 「ルール」タブの名前が「検索ルール」に変更されました
* 「トレンド」でランク付けする場合、次の中から選択できるようになりました。
   * 3 日（デフォルト）
   * 14 日
   * 30 日
* 「イベント」（ブースト/ピン/ベリー/非表示）の名前が「手動ランキング」に変更されました。
* 「ランキングタイプ」は「インテリジェントランキング」に名前が変更されました
...
* 製品リストウィジェットが更新され、 [カタログサービス API](https://developer.adobe.com/commerce/webapi/graphql/schema/catalog-service/queries/product-search/).
