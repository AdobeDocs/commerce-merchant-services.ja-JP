...
- **[購入回数](#orders-module)** — ストアに対しておこなわれたすべての注文のリストを表示して、各注文の注文数とステータスを全員が把握できるようにします。
...
関連付けがピッキングを開始し、ピッキングする品目を選択すると、サイズ、色、数量、価格、説明、適合、SKU/UPC など、関連するすべての品目情報が表示されます。 受注のピッキング時に、次の 2 つの処理を使用できます。
...
自由形式のテキストバーを使用して、注文の位置を示すことができます。カウンターの下、奥の部屋、または場所 B3 — すべて操作に基づいて。

また、関連者が別の場所に配置する必要がある場合は、アプリ内のステージング場所を簡単に更新できます。

## ハンドオフモジュール

The [!UICONTROL Hand Off] モジュールは、どの顧客が注文を受け取るためにチェックインしたかを表示し、注文がステージングされる場所と、注文を待機する場所を示します。 お客様は、店内または駐車場の外/縁日で受け取ることができます。 正確な場所に関する情報は、チェックイン後にこのモジュールに表示されます。

顧客への引き渡しの準備ができた注文を選択したら、 見つからなかった品目、顧客待ち場所、待ち時間、注文のステージング場所、お客様の電話番号（ある場合）を含む注文情報を、すべてのユーザーが確認できます。
