---
title: 一般設定
description: '"一般設定を構成して有効にします [!DNL Store Fulfillment] お客様のストアの グローバル拡張機能の設定、ログのシステム設定、データの同期、セキュリティを設定します。 Adobe Commerceとストアフルフィルメントサービスの統合を可能にするための主要なデータを提供します。」'
role: User, Admin
level: Intermediate
...
この設定を完了するには、 **店舗→構成→サービス→ Store Fulfilment by Walmart Commerce Technologies**.
